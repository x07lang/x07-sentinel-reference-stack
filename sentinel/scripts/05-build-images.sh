...
docker build -f "${ROOT_DIR}/apps/orders-api/Dockerfile"   --platform "${DOCKER_PLATFORM}"   --build-arg "X07_TAG=${X07_TAG}"   --build-arg "APP_VERSION=${APP_VERSION}"   -t "${IMAGE_PREFIX}/orders-api:${APP_VERSION}"   "${ROOT_DIR}"
